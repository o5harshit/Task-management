...
export default function ProjectsGrid({ projects }) {
  const handleDelete = async(id) => {
    const res = await apiClient.post(`${PROJECT_ROUTES}/projects`,{id : id},{withCredentials : true});
    console.log(res);
  }
  return (
                ...
                <CardContent className="flex flex-col h-full space-y-4 p-0">
                  ...
                  <div className="mt-auto flex flex-col gap-4">
                    ...
                    <Link to={`/projects/${project._id}/task`}>
                      <Button className="bg-purple-600 hover:bg-purple-700 text-white transition rounded-sm cursor-pointer">
                        View Tasks
                      </Button>
                    </Link>
                    <Link to={`/projects/${project._id}/add-task`}>
                      <Button className="bg-purple-600 hover:bg-purple-700 text-white transition rounded-sm cursor-pointer">
                        Add more Task
                      </Button>
                    </Link>
